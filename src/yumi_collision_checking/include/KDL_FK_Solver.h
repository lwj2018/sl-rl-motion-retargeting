...
class KDL_FK_Solver
{
  public:
    ...
    Matrix3d return_shoulder_ori(KDL::ChainFkSolverPos_recursive &fk_solver, Matrix<double, 7, 1> q_cur, bool left_or_right);
    Matrix3d return_ori(KDL::ChainFkSolverPos_recursive &fk_solver, Matrix<double, 7, 1> q_cur, unsigned int ind);
    Vector3d return_pos(KDL::ChainFkSolverPos_recursive &fk_solver, Matrix<double, 7, 1> q_cur, unsigned int ind);
    ...
    unsigned int r_num_joint2_seg = 0;
  ...
  private:
    ...
    // params
    const std::string URDF_FILE = "/home/liweijie/projects/sl-rl-motion-retargeting-ws/yumi_with_hands.urdf";
